...
### Testing
- **Mandatory Testing**: All `.go` files must have a corresponding `_test.go` file. Untested code is not allowed, except for auto-generated files (e.g., `*.gen.go`, `*.pb.go`).
...
### Testing
- **Mandatory Testing**: All `.ts` files must have a corresponding `.test.ts` file located in the same directory (sibling tests). Untested code is not allowed, except for auto-generated files (e.g., from `openapi-fetch`).
...
- [ ] **Is it Tested?** Do all new files have corresponding tests?
